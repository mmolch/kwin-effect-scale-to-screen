...
void ScaleToScreen::start()
{
    qCDebug(lcScaleToScreen) << "start()";
    ...
    if (!m_state.isSuspended) {
        ...
        connect(m_state.window->window(), &Window::decorationPolicyChanged, this, &ScaleToScreen::onDecorationPolicyChanged);
    }
...
void ScaleToScreen::stop()
{
    qCDebug(lcScaleToScreen) << "stop()";
    suspend();

    disconnect(m_state.window->window(), &Window::decorationPolicyChanged, this, &ScaleToScreen::onDecorationPolicyChanged);
    m_state.window->window()->setNoBorder(m_state.originalNoBorder);
    m_state.window = nullptr;
}

void ScaleToScreen::suspend()
{
    if (m_state.isSuspended) {
    ...
    qCDebug(lcScaleToScreen) << "suspend()";
...
void ScaleToScreen::scaleActiveWindow()
{
    ...
    if (m_state.window != active) {
        if (m_state.isScaling || m_state.isSuspended) {
            stop();
        }
...
void ScaleToScreen::onDecorationPolicyChanged()
{
    if (!m_state.window) {
        return;
    }

    m_state.window->window()->setNoBorder(true);
}
